...
const CACHE_NAME = CACHE_PREFIX + '20260910004138';
...
/** Guest deep links are not real files â€” GitHub Pages serves 404.html (the SPA). */
  ...
  event.waitUntil(
    (async () => {
      ...
      if (ready) {
        ...
        ngmySwQuiet('[ngmy-sw] keep prior cache â€” new shell not ready yet');
      }
...
self.addEventListener('fetch', (event) => {
  ...
  // POST /api/sync â€” same-origin proxy; DevTools never shows bright-handler.
  ...
  // /api/rest/v1/* â€” same-origin PostgREST proxy (hides supabase.co URLs).
      ...
      if (event.request.mode === 'navigate') {
        ...
        if (self.navigator.onLine) {
          try {
            ...
            // shell) with status 404 â€” still serve that HTML, or the cached shell.
