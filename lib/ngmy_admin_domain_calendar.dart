...
class NgmyDomainReminderStore {
  ...
  static const _shownPrefsKey = 'ngmy_domain_renewal_reminder_shown_v1';
  ...
  static Future<void> save(NgmyDomainReminderSettings settings) async {
    ...
    if (!_sameDay(prev.expiryDate, settings.expiryDate)) {
      await prefs.remove(_shownPrefsKey);
    }
  }
  ...
  static String _expiryKey(DateTime expiry) =>
      '${expiry.year}-${expiry.month.toString().padLeft(2, '0')}-${expiry.day.toString().padLeft(2, '0')}';

  /// Returns true if this reminder type was already shown for this expiry date.
  static Future<bool> wasShown(String reminderId) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final raw = prefs.getString(_shownPrefsKey);
      if (raw == null || raw.isEmpty) return false;
      final decoded = jsonDecode(raw);
      if (decoded is! List) return false;
      return decoded.map((e) => e.toString()).contains(reminderId);
    } catch (_) {
      return false;
    }
  }

  static Future<void> markShown(String reminderId) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final raw = prefs.getString(_shownPrefsKey);
      final ids = <String>{};
      if (raw != null && raw.isNotEmpty) {
        final decoded = jsonDecode(raw);
        if (decoded is List) ids.addAll(decoded.map((e) => e.toString()));
      }
      ids.add(reminderId);
      await prefs.setString(_shownPrefsKey, jsonEncode(ids.toList()));
    } catch (_) {}
  }
}
...
/// Admin-only: notify once 1 day before expiry, and once on expiry day (not every app open).
Future<void> ngmyRunDomainRenewalReminderIfNeeded({
  required bool isAdmin,
  required Future<void> Function(String title, String body, {required String tag}) onNotify,
}) async {
  if (!isAdmin) return;
  ...
  final expiryKey = NgmyDomainReminderStore._expiryKey(expiry);

  if (daysUntil == 1) {
    final id = 'day_before_$expiryKey';
    if (await NgmyDomainReminderStore.wasShown(id)) return;
    await onNotify(
      'ngmy.org renews tomorrow',
      'Your domain expires on ${ngmyFormatDomainDate(expiry)}. Renew at your registrar so NGMY stays online.',
      tag: id,
    );
    await NgmyDomainReminderStore.markShown(id);
  } else if (daysUntil == 0) {
    final id = 'expiry_day_$expiryKey';
    if (await NgmyDomainReminderStore.wasShown(id)) return;
    await onNotify(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                ...
                '• One alert 1 day before expiry (e.g. 10 days left → rings on day 9)\n'
                '• One alert on expiry day\n'
                '• Will not repeat every time you open the app',
