...
class _NgmyGuestCivicEnrollScreenState extends State<NgmyGuestCivicEnrollScreen> with TickerProviderStateMixin {
  ...
  String? _loadError;
  String? _registryId;
  /// Set after triple-tapping the NGMY logo + entering a Civic Registry ID.
  String _unlockedRegistryId = '';
  bool _logoDialogOpen = false;
  int _logoTapCount = 0;
  DateTime? _logoTapAt;
  ...
  Future<void> _submit() async {
    ...
    if (!_selfEnrollOpen && _registrarToken.isEmpty && unlockedId.isEmpty) {
    ...
    try {
      // Logo-unlocked Registry ID → update existing profile (never create).
      if (unlockedId.isNotEmpty) {
        final updated = await ngmyCivicGuestSelfUpdate({
          'mode': 'profile',
          'fullName': fullName,
          'homeAddress': address,
          'phone': phone,
          'registryId': unlockedId,
        ...
        setState(() {
          _submitting = false;
          _done = true;
          _registryId = updated.registryId ?? unlockedId;
          _members = const [];
        });
        return;
      }

      // New enrollment — if already registered, silently update family size.
  ...
  Future<void> _promptRegistryIdUnlock() async {
    if (_logoDialogOpen || !mounted) return;
        ...
        child: AlertDialog(
          ...
          actions: [
            ...
            TextButton(
              ...
              child: const Text('OK', style: TextStyle(color: _kAccent, fontWeight: FontWeight.w800)),
            ),
          ],
        ),
      ),
    ...
    setState(() => _unlockedRegistryId = id);
  }
          ...
          Expanded(
            child: ListView(
              ...
              children: [
                ...
                if (_unlockedRegistryId.isNotEmpty) ...[
                  const SizedBox(height: 12),
                  _glassField(
                    child: TextField(
                      controller: _dobC,
                      ...
                      decoration: _dec('Tarehe ya kuzaliwa').copyWith(hintText: '01/15/1990'),
                    ),
                  ),
                ],
                const SizedBox(height: 12),
