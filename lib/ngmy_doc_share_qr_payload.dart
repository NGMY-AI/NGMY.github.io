...
class NgmyDocShareQrPayload {
  ...
  /// On-screen QR size — short LAN codes get big modules; dense codes cap at [maxSide].
  static double qrSizeForData(String data, {double maxSide = 268}) {
    final len = data.length;
    if (len < 120) return maxSide.clamp(240, 268);
    if (len < 350) return (maxSide - 8).clamp(232, 260);
    if (len < 800) return (maxSide - 16).clamp(224, 252);
    return (maxSide - 24).clamp(216, 248);
  }
  ...
  static bool showCenterLogo(String data) => data.length <= 900;

  static double logoSizeFraction(String data) {
    if (data.length <= 120) return 0.18;
    if (data.length <= 400) return 0.14;
    return 0.11;
  }
