...
String? _slugFromPath(String path) {
  final match = RegExp(r'/local-menu/([^/?#]+)', caseSensitive: false).firstMatch(path);
  if (match != null) {
  ...
  for (var i = 0; i < segments.length - 1; i++) {
    if (segments[i].toLowerCase() == 'local-menu') {
...
String? ngmyReadLocalMenuSlugFromLaunchUrl() {
  final uri = Uri.parse(html.window.location.href);
  final fromPath = _slugFromPath(uri.path);
  if (fromPath != null && fromPath.isNotEmpty) {
    try {
      html.window.sessionStorage['ngmy_guest_local_menu_slug'] = fromPath;
    } catch (_) {}
    return fromPath;
  }

  try {
    final stored = html.window.sessionStorage['ngmy_guest_local_menu_slug'];
    if (stored != null && stored.trim().isNotEmpty) return stored.trim().toLowerCase();
  } catch (_) {}

  final slug = uri.queryParameters['ngmy_local_menu']?.trim();
  if (slug != null && slug.isNotEmpty) return slug.toLowerCase();

  final hash = uri.fragment.trim().replaceFirst(RegExp(r'^/?'), '');
  if (hash.startsWith('local-menu/')) {
    final fromHash = hash.substring(11).split('/').first.trim();
