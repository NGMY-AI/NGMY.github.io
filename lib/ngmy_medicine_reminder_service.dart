...
Future<bool> _alreadyFiredToday(
  String userEmail,
  String medicineId,
  String slot,
) async {
...
Future<List<(NgmyMedicineEntry, String)>> ngmyMedicinesDueNow({
  ...
  for (final m in items) {
    ...
    final times = m.reminderTimes.isNotEmpty
        ? m.reminderTimes
        : ngmyDefaultMedicineReminderTimes(m.timesPerDay);
...
void ngmyStartMedicineReminderWatcher(String userEmail) {
  ...
  _medicineReminderPollTimer = Timer.periodic(
    const Duration(seconds: 30),
    (_) => unawaited(_pollMedicineReminders()),
  );
...
Future<void> _pollMedicineReminders() async {
  ...
  await showNgmyMedicineReminderAlert(
    ctx,
    medicine: medicine,
    timeSlot: slot,
    userEmail: email,
  ...
  try {
    await showGeneralDialog<void>(
      ...
      transitionBuilder: (ctx, anim, _, child) {
        // Pops up like a card sliding in from the bottom edge — not a
        // full-screen takeover — so the home screen stays visible (blurred)
        // above and below it.
...
class _MedicineReminderOverlay extends StatefulWidget {
  const _MedicineReminderOverlay({
  ...
  @override
  State<_MedicineReminderOverlay> createState() =>
      _MedicineReminderOverlayState();
}
                        ...
                        child: Container(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(
                                  'Scheduled for ${_formatSlot(widget.timeSlot)}',
                                  style: TextStyle(
