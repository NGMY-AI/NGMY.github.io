...
/// Bundled locally (assets/images/swahili/) so lesson pictures work fully
/// offline — no network request, ever.
String ngmySwahiliPhoto(String photoId, {int w = 960, int h = 720}) {
  return 'assets/images/swahili/${photoId.replaceFirst('photo-', '')}.jpg';
}
...
String ngmySwahiliPhotoBody(String photoId) {
  return 'assets/images/swahili/${photoId.replaceFirst('photo-', '')}_portrait.jpg';
}
...
final Map<String, SwahiliWordVisual> _kExactBySwahili = {
  ...
  'naweza kuongea kiingereza': SwahiliWordVisual(imageUrl: ngmySwahiliPhoto('photo-1542229538-4d8784038379'), caption: 'I can speak English'),
  'ninajifunza kiingereza': SwahiliWordVisual(imageUrl: ngmySwahiliPhoto('photo-1553729784-e91953dec042'), caption: 'I am learning English'),
  ...
  'mimi ni mzungumzaji wa kiingereza': SwahiliWordVisual(imageUrl: ngmySwahiliPhoto('photo-1542229538-4d8784038379'), caption: 'I am an English speaker'),
  'sasa ninaongea kiingereza': SwahiliWordVisual(imageUrl: ngmySwahiliPhoto('photo-1769708046949-108aa733f753'), caption: 'Now I speak English'),
};

final _kDefaultVisual = SwahiliWordVisual(
  imageUrl: ngmySwahiliPhoto('photo-1456513080510-7bf3a84b82f8'),
  caption: 'Learn',
);

/// Every curriculum word has an exact entry in [_kExactBySwahili] (verified
/// offline-coverage: 0 words fall through) — this default only guards
/// against a future word being added without one.
SwahiliWordVisual resolveSwahiliWordVisual({required String swahili, required String english}) {
  ...
  return _kExactBySwahili[swKey] ?? _kExactBySwahili[swBare] ?? _kDefaultVisual;
}

/// Full-screen photo viewer — tap any lesson picture to enlarge. Bundled
/// local assets are a single fixed resolution, so no URL upscaling is
/// needed — InteractiveViewer handles the zoom.
