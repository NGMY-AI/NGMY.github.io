...
class _NgmyStudioLogoAnimState extends State<NgmyStudioLogoAnim> with SingleTickerProviderStateMixin {
  ...
  Widget? _imageWidget() {
    final isCircle = widget.shape == NgmyVideoSlotShape.circle;
    final fit = isCircle ? BoxFit.contain : BoxFit.cover;

    if (widget.imageBytes != null && widget.imageBytes!.isNotEmpty) {
      return Image.memory(widget.imageBytes!, fit: fit, gaplessPlayback: true);
    }

    final fromPath = path_image.ngmyStudioImageFromPath(widget.filePath);
    if (fromPath != null) {
      return FittedBox(fit: fit, child: fromPath);
    }

    if (widget.networkUrl != null && widget.networkUrl!.startsWith('http')) {
      return Image.network(widget.networkUrl!, fit: fit);
    }
          ...
          child: img == null
              ...
              : _ctrl == null
                  ...
                  builder: (_, child) {
                    final t = _ctrl!.value;
                    final isCircle = widget.shape == NgmyVideoSlotShape.circle;
                    final scale = isCircle ? 1.0 : (0.94 + t * 0.04);
                    final dy = isCircle ? 0.0 : (t - 0.5) * 3;
                          ...
                          child: ClipPath(
                            ...
                            child: DecoratedBox(
                              decoration: BoxDecoration(
                                color: isCircle ? const Color(0xFF0F1423) : Colors.transparent,
                              ),
                              child: child,
                            ),
                          ),
